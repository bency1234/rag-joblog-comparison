...
```bash
joblog.ai$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
joblog.ai$ sam logs -n HelloWorldFunction --stack-name "joblog.ai" --tail
```
...
```bash
joblog.ai$ pip install pytest pytest-mock --user
joblog.ai$ python -m pytest tests/ -v
```
